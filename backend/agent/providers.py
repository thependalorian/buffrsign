...
def get_llm_model(model_choice: Optional[str] = None, user_tier: Optional[str] = None) -> OpenAIModel:
    """
    Get LLM model configuration based on environment variables and user tier.
    
    Args:
        model_choice: Optional override for model choice
        user_tier: User tier ('standard' or 'pro') to determine Groq model
    
    Returns:
        Configured OpenAI-compatible model (Groq or OpenAI)
    """
    # Check if we should use Groq
    groq_api_key = os.getenv('GROQ_API_KEY')
    if groq_api_key and user_tier:
        # Use Groq models based on user tier
        if user_tier == 'pro':
            llm_choice = os.getenv('GROQ_LLM_PRO', 'llama-3.1-70b-versatile')
        else:
            llm_choice = os.getenv('GROQ_LLM_STANDARD', 'llama-3.1-8b-instant')
        
        base_url = os.getenv('GROQ_BASE_URL', 'https://api.groq.com/openai/v1')
        api_key = groq_api_key
    else:
        # Fallback to OpenAI or other providers
