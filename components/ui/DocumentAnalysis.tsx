...
import { DocumentAnalysisProps, type DocumentAnalysis } from '@/lib/design-system';
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {analysis.extracted_fields.map((field, _index) => (
              <div key={_index} className="flex justify-between items-center p-2 bg-muted rounded">
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          ...
          <div className="space-y-2">
            {analysis.signature_locations.map((field, _index) => (
              <div key={_index} className="flex items-center justify-between p-3 bg-chart-1/10 rounded-lg">
                <div>
                ...
                <div className="flex items-center space-x-2">
                  {field.required && (
                    <span className="px-2 py-1 text-xs bg-chart-5/20 text-chart-5 rounded">
                      Required
                    </span>
                  )}
                  <button className="px-3 py-1 text-sm bg-chart-1 text-white rounded hover:bg-chart-1/90">
                    Sign
                  </button>
                </div>
              </div>
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          ...
          <div className="flex items-center space-x-2">
            <div className={cn(
              ...
              analysis.compliance_score >= 80 ? 'bg-chart-2' : 
              analysis.compliance_score >= 60 ? 'bg-chart-3' : 'bg-chart-5'
            )} />
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={cn(
                ...
                analysis.risk_assessment.overall_risk === 'low' ? 'bg-chart-2/20 text-chart-2' :
                analysis.risk_assessment.overall_risk === 'medium' ? 'bg-chart-3/20 text-chart-3' :
                analysis.risk_assessment.overall_risk === 'high' ? 'bg-chart-5/20 text-chart-5' :
            ...
            <div className="mt-3">
              ...
              <div className="space-y-1">
                {analysis.risk_assessment.risk_factors.map((factor, _index) => (
                  <div key={_index} className="text-sm text-muted-foreground">
                    • {factor.description}
                  </div>
                ))}
              </div>
        ...
        <div className="bg-card p-4 rounded-lg border border-border">
          ...
          <div className="space-y-2">
            {analysis.ai_insights.map((insight, _index) => (
              <div key={_index} className="p-3 bg-chart-4/10 rounded-lg">
                <div className="flex items-start space-x-2">
                  <svg className="h-5 w-5 text-chart-4 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                  </svg>
                  <div>
                    <p className="text-sm font-medium text-chart-4">{insight.category}</p>
                    <p className="text-sm text-chart-4/80">{insight.insight}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            {analysis.recommendations.map((rec, _index) => (
              <div key={_index} className="flex items-start space-x-2">
                <div className={cn(
                  ...
                  rec.priority === 'high' ? 'bg-chart-5' :
                  rec.priority === 'medium' ? 'bg-chart-3' : 'bg-chart-2'
                )} />
                <div>
